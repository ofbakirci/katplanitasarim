...
| **C5 / C6** | BU OTURUMDA YOK | — | C5 (mobilya headless parity + prompt + A/B kanıt) + C6 (depth map, stretch) → sonraki oturum. |
| **C7** A/B varsayılanı | KULLANICIYA SORULDU | — | Kamera render A(prompt)+B(snapshot) şu an PARALEL (`render_method:'both'` doğrulandı, 2× maliyet). Öneri: B varsayılan, A istek-üzerine. Onay gelirse AYRI iş kalemi (bu oturumda UYGULAMA YOK). |
