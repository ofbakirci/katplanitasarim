...
| **A7** checks.js kural-tablosu | BEKLIYOR (stretch) | — | Büyük; ayrı oturum. **A2'den devir:** checks.js sabitleri (ör. checks.js:177 salon-şişme `45`) bilinçli ertelendi → A7 kapsamı. |
...
| **C5** mobilya render sinyali | BITTI (paralı adım 5 BEKLIYOR) | `view3d.js`/`io.js` diff yok (io zaten export ediyordu); tracked `view3d.js`(C6 ile) + gitignored `iso_render.py`/`render-server.js` → `DURUM.md §8` | Tarayıcı iso PNG davranışı belgelendi (tutarlı, düzeltme yok). **Headless parity:** `iso_render.py` furniture[]'tan sade kutu çizer (FURN_DIM port + furn_footprint, koyu kategori tonu, `__w/__d`, mobilyasız=bayt-özdeş). **KANIT:** 126-item furnished plan tarayıcı vs headless → `tests_render/out/C5_parity_kiyas.png`; **sayısal parite 0 px** (headless footprint = tarayıcı polygon_px). **Prompt:** isoPrompt'a "furniture blocks are real positions, dress in place" koşullu eklendi (kamera B zaten vardı). Adım 5 (mobilyalı iso→nano prompt-ile/olmadan) = PARALI → BEKLIYOR (onay+token). |
...
| **D** | BEKLIYOR | — | ML ranker — ayrı oturum. |
