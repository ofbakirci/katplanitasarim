...
1. **Yeni il sağlayıcıları (Bursa/Antalya/Kocaeli/Konya…)** — aynı `IMAR_PROVIDERS` kalıbı. ArcGIS REST açık-anonim çoğu büyükşehirde muhtemel; iş akışı: `izmir-imar-recon` workflow'unu il adıyla tekrarla (çok-açılı keşif→curl doğrulama→reçete) → provider ekle. İzmir/Ankara reçetesi şablon.
