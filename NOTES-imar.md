...
1. **Yeni il sağlayıcıları (Antalya/Kocaeli/Konya…)** — aynı `IMAR_PROVIDERS` kalıbı. İş akışı: `izmir-imar-recon` workflow'unu il adıyla tekrarla (çok-açılı keşif→curl doğrulama→reçete) → provider ekle. İzmir/Ankara reçetesi şablon. **FİZİBİLİTE KRİTERİ (Bursa'dan öğrenildi):** Bu app STATİK/proxy-siz (tek-dosya/Capacitor) → bir il ANCAK (a) CORS-açık (Origin yansıtan ya da `*`) bir uçtan, (b) ANONİM, (c) YAPISAL yapılaşma hakkı (TAKS/KAKS/kat öznitelikli) veriyorsa eklenebilir. CORS-kapalı sistemler (çoğu Netcad KEOS ilçe app'i) tarayıcıdan çekilemez → server-side proxy gerekir, bu app'te YOK.
