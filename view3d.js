...
(function(){
  ...
  function currentCamCtx(){
    return { floor:(typeof activeFloor!=='undefined')?activeFloor:0, block:(typeof activeBlock!=='undefined')?activeBlock:0 };
  }
  function stampCamCtx(c){ const ctx=currentCamCtx(); c.__floor=ctx.floor; c.__block=ctx.block; return c; }
  // damga yoksa (legacy kayıt — eski projeden gelen/testte elle kurulan) HER bağlamda eşleşir → mevcut
  //   projeler kırılmaz (ilk yeniden-yazımda — taşı/aim/prompt düzenle vb. — damgalanır, bkz. camList.push noktaları).
  function camCtxMatch(c){
  ...
  function ensureOverlay(){
    ...
    overlay.innerHTML =
      ...
      '<div id="v3dViewCtl" style="position:absolute;right:14px;bottom:14px;z-index:4;display:flex;flex-direction:row;align-items:flex-end;gap:8px">'+
        ...
        '<div style="display:flex;flex-direction:column;align-items:center;gap:6px">'+
          ...
          orbWidgetHTML()+
        '</div>'+
        // İŞ 5b: GENERİK GÖRÜNÜM-KİLİDİ düğmesi — orb/preset kümesinin YANINA, v3dLockBtn'den (kuşbakışı orbit
        //   kilidi) AYRI id+ikon. view3d "plan boyama" kavramını BİLMEZ: yalnız kayıtlıysa görünür (setViewLockCallback),
        //   tıklanınca callback'i tetikler — anlamı (neyin kilitlendiği) tamamen çağıranda (prototip). Varsayılan gizli.
        '<div id="v3dViewLockWrap" style="display:none;flex-direction:column;align-items:center;gap:6px">'+
          '<button id="v3dViewLockBtn" style="width:38px;height:38px;border:0;border-radius:'+UIPAL.r10+';background:'+UIPAL.panel+';color:'+UIPAL.inkAcc+';cursor:pointer;display:flex;align-items:center;justify-content:center;backdrop-filter:'+UIPAL.blur+';box-shadow:'+UIPAL.shadowSm+'"></button>'+
        '</div>'+
    ...
    overlay.addEventListener('click',function(e){
      ...
      const xa=t.getAttribute&&t.getAttribute('data-extact'); if(xa){ setExtPlaceAction(extPlaceAction===xa?'none':xa); return; }  // A4: Yön/Taşı toggle (zemine-tıkla eylemi)
    ...
    const vlb=overlay.querySelector('#v3dViewLockBtn'); if(vlb) vlb.addEventListener('click',toggleViewLock);
    renderRail(); renderDrawer(); renderViewLockBtn();   // İŞ 5b: overlay ilk kurulduğunda ÖNCEDEN kayıtlı callback varsa buton görünsün
  }
  ...
  function attachOrbit(o,d){
    ...
    function wheel(e){ e.preventDefault(); if(!c.enabled) return;   // kilitliyken tekerlek-zoom da kapalı (panel zoom slider'ı açık kalır)
      ...
      if(exteriorMode && extActive>=0 && !extGhost && !extDrag) return;   // A4: dış modda drone seçili → tekerlek yaw döndürür, zoom yapma
  ...
  // ── İŞ 5b: GENERİK GÖRÜNÜM-KİLİDİ tetikleyicisi ─────────────────────────────────────────────
  //   view3d "plan boyama" kavramını BİLMEZ — yalnız (a) bir callback kayıtlıysa küçük buton belirir,
  //   (b) tıklanınca fn(getView()) çağrılır (kilitle), (c) ikinci tık fn(null) (kaldır). Anlam ÇAĞIRANDA
  //   (mesken/02_PROTOTIP: state.lockedViews[blok] = view). setViewLockState dışarıdan görsel durumu
  //   düzeltmek için (örn. eski "Açıyı kilitle → Boya" dialog akışıyla senkron) — buton kendi iç toggle'ını
  //   da tutar ama dış senkron her zaman KAZANIR (son çağrılan render'ı belirler).
  let viewLockCallback=null, viewLockOpt=null, viewLockActive=false;
  function viewLockBtnEl(){ return overlay&&overlay.querySelector('#v3dViewLockBtn'); }
  function renderViewLockBtn(){
    const wrap=overlay&&overlay.querySelector('#v3dViewLockWrap'), b=viewLockBtnEl();
    if(!wrap||!b) return;
    ...
    b.style.color=viewLockActive?UIPAL.onAcc:UIPAL.inkAcc;
  }
  // fn(view,opt): view=null|{position,target,up,fov} — çağıran "kilitli" kavramını burada yönetir.
  function setViewLockCallback(fn,opt){
    viewLockCallback=(typeof fn==='function')?fn:null;
    viewLockOpt=opt||null;
    renderViewLockBtn();
  }
  ...
  function toggleViewLock(){
    ...
    try{ viewLockCallback(viewLockActive?getView():null); }catch(e){}
    renderViewLockBtn();
  }
  ...
  function extOtherBlocks(){
    ...
    try{ if(typeof siteOn==='function' && siteOn() && typeof blocks!=='undefined' && Array.isArray(blocks)){
      ...
      blocks.forEach(function(b,i){
        ...
        p.idx=i; p.snap=b;                              // C1: tam blok cephesi map'ini snapshot'tan türetmek için
  ...
  function buildExtFloorGroup(map, contourM, holesM, floorH, mats, showcase, edgeShift){
    ...
    // bir kapalı halka (ring) kenarları boyunca solid duvar + pencere/balkon boşlukları oyar. Dış kontur VE
    //   (C3) avlu kuyu halkaları için ORTAK — avlu kenarları da pencere alır (winSegs avlu penceresini de içerir).
    //   refInside: iç normalin gösterdiği referans nokta yönü. Dış konturda poligon merkezi (içeri bakar);
    //   AVLU halkasında merkez BOŞLUKtur → normal DIŞA (odaya) baksın diye ters işaret (want= -1).
    function carveRing(ring, wantInsideSign){
      let ccx=0,ccz=0; ring.forEach(function(p){ ccx+=p[0]; ccz+=p[1]; }); ccx/=ring.length; ccz/=ring.length;
      for(let i=0;i<ring.length;i++){
        const a=ring[i], b=ring[(i+1)%ring.length];
        const dx=b[0]-a[0], dz=b[1]-a[1], len=Math.hypot(dx,dz); if(len<0.2) continue;
        const ux=dx/len, uz=dz/len, ang=-Math.atan2(dz,dx);
        // iç normal: ring merkezine göre; avluda merkez boşluk → wantInsideSign=-1 ile odaya çevrilir
        let nx=-uz, nz=ux; const mmx=(a[0]+b[0])/2, mmz=(a[1]+b[1])/2;
        const towardCenter=(ccx-mmx)*nx+(ccz-mmz)*nz;
        if(towardCenter*wantInsideSign<0){ nx=-nx; nz=-nz; }
        const nIn=[nx,nz];
        ...
        //   avlu halkasında kenar uçlarında WALL_T taşma (kuyu köşesi kapansın) → seg başı/sonu ±WALL_T/2 uzatılır
        const overrun=(wantInsideSign<0)?WALL_T/2:0;
        function seg(s0,s1){ if(s1-s0<0.05) return; const S0=s0-((s0<=0.001)?overrun:0), S1=s1+((s1>=len-0.001)?overrun:0);
        ...
        const nOut=[-nIn[0],-nIn[1]];   // dış cephe yönü (avluda: avlu boşluğuna bakar → denizlik avluya çıkar)
    ...
    carveRing(cont, 1);   // dış kontur — normal içeri (poligon merkezine) bakar
    // 2b) AVLU KUYU DUVARLARI — kabukta avlu DELİK; deliğin çevresi katta avlu duvarıyla kapanır. C3: avluya bakan
    //   yaşam-odası cepheleri de pencere alır (winSegs avlu penceresini içerir); kalan kısım düz kuyu duvarı.
    (holesM||[]).forEach(function(h){ carveRing(h, -1); });   // avlu halkası — normal boşluktan DIŞA (odaya) bakar
  ...
  function buildExterior(){
    ...
    // İŞ B3: KAT-BAŞINA DIŞ CEPHE — floorsOn() + en az bir ZİYARET EDİLMİŞ kat (floorFacadeGroupsForTest
    //   sig!=null) varsa her kat KENDİ snapshot'ından (floorSnapshotAt→blockFloorplanMap) cephe kurar; aynı
    //   imzalı ARDIŞIK katlar (tip-kat) TEK proto'yu paylaşır (sig→proto önbelleği). Ziyaret edilmemiş kat
    //   aşağıdaki `proto`'ya (mevcut tekil-yol) güvenli düşer. perFloorOn=false (floorsOn() kapalı/hiç
    //   ziyaret yok) → floorProtos=null, stackShell BYTE-AYNI eski tekil-proto yoluna devam eder.
    let facadeGroups=null, perFloorOn=false;
    try{
      if(typeof floorsOn==='function' && floorsOn() && typeof floorFacadeGroupsForTest==='function'){
        facadeGroups=floorFacadeGroupsForTest();
        perFloorOn=facadeGroups.some(function(g){ return g.sig!=null; });
      }
    }catch(e){ facadeGroups=null; perFloorOn=false; }
    // C2: aktif görüntülenen kat ZEMİN ticari ise ÜST katlar için proto'yu üst (konut) kat map'inden kur
    //   (aksi halde ticari vitrin TÜM katlara klonlanırdı). Aktif kat zemin değilse map = üst kat zaten.
    //   İŞ B4: kat-başına yol (perFloorOn) AKTİFKEN bu düzeltme GEREKSİZ — her kat (ticari zemin dahil,
    //   aşağıdaki groundProto ile) zaten KENDİ snapshot'ından kurulur; `proto` yalnız ZİYARET EDİLMEMİŞ
    //   katların düşüşü için kalır, düz `map` yeter. Kat-başına yol KAPALIYKEN (floorsOn() false / hiç
    //   ziyaret yok) eski güvenli-düşüş AYNEN çalışmaya devam eder (bilinen sınır: perFloorOn açıkken
    //   ziyaret edilmemiş ÜST kat + aktif görüntülenen ZEMİN ticari kombinasyonunda düşüş düzeltmesiz —
    //   kabul edilen basitleştirme, İş B4 kapsamı).
    let upperMap=map;
    if(!perFloorOn){
      try{
        if(typeof groundFloorSnapshot==='function'){
          const gfChk=groundFloorSnapshot();
          if(gfChk && gfChk.usage==='ticari' && typeof activeFloor!=='undefined' && typeof zeminIdx==='function'
             && activeFloor===zeminIdx() && typeof upperFloorSnapshot==='function'){
            const uf=upperFloorSnapshot();
            if(uf && uf.snap && typeof blockFloorplanMap==='function'){ const um=blockFloorplanMap(uf.snap); if(um) upperMap=um; }
          }
        }
      }catch(e){ upperMap=map; }
    }
    const proto=buildExtFloorGroup(upperMap, contour, holes, floorH, mats);
    if(extActiveIsFull()){
      // İŞ B3 (devam): sig→proto önbellekli kat-başına proto dizisi (f=0..fc.above-1) — yalnız TAM kabuk
      //   çizilecekse kurulur (hayalet dalı bu proto'ları hiç kullanmaz, gereksiz mesh inşası önlenir).
      //   Bilinen sınır (İş B5): katlar arası pts (dış kontur) farklıysa buildExtFloorGroup hâlâ ORTAK
      //   `contour`/`holes` (2B-aktif bloğun GÜNCEL konturu) ile kurulur — yalnız pencere/balkon boşlukları
      //   o katın MAP'inden gelir; eşleşmeyen pencere carveRing toleransında sessizce düşer (çok-kontur
      //   desteği ayrı iş, kapsam dışı).
      let floorProtos=null;
      if(perFloorOn && typeof zeminIdx==='function' && typeof floorSnapshotAt==='function'){
        floorProtos=[];
        const sigProto=new Map();   // sig → o gruptan (temsilci kattan) kurulmuş proto
        for(let f=0; f<fc.above; f++){
          const k=zeminIdx()+f;
          const grp=(facadeGroups||[]).filter(function(g){ return g.idxs.indexOf(k)>=0; })[0];
          const sig=grp?grp.sig:null;
          if(sig==null){ floorProtos[f]=null; continue; }   // ziyaret edilmemiş → aşağıda `proto`'ya düşer
          if(sigProto.has(sig)){ floorProtos[f]=sigProto.get(sig); continue; }
          let fp=null;
          try{
            const snap=floorSnapshotAt(grp.rep);
            const fMap=snap?blockFloorplanMap(snap):null;
            if(fMap) fp=buildExtFloorGroup(fMap, contour, holes, floorH, mats);
          }catch(e){ fp=null; }
          sigProto.set(sig, fp);
          floorProtos[f]=fp;
        }
      ...
      try{
        ...
        if(gf && gf.snap && gf.usage==='ticari'){
          // zemin ticari cephesi: aktif kat zeminse map ZATEN zemin (ayrı hesaba gerek yok), değilse snapshot'tan.
          const gMap=(typeof activeFloor!=='undefined' && typeof zeminIdx==='function' && activeFloor===zeminIdx())
            ? map
            : ((typeof blockFloorplanMap==='function')?blockFloorplanMap(gf.snap):null);
          if(gMap){ const gGrp=buildExtFloorGroup(gMap, contour, holes, floorH, mats, true);   // vitrin işareti
    ...
    others.fulls.forEach(function(fb){
      ...
      // C1: bloğun KENDİ snapshot'ından TAM (pencereli/balkonlu) cephe map'i türet (yan-etkisiz). Başarısızsa
      //   (planı yok / hesap hatası) eski penceresiz sade kabuğa güvenli düş → render kırılmaz.
      let fMap=null; try{ if(typeof blockFloorplanMap==='function' && fb.snap) fMap=blockFloorplanMap(fb.snap); }catch(e){ fMap=null; }
      // blok avlu delikleri (snapshot courtyards → dünya-metre halkalar)
      const fHoles=(fb.snap && Array.isArray(fb.snap.courtyards))
        ? fb.snap.courtyards.filter(function(a){ return a&&a.poly&&a.poly.length>=3; }).map(function(a){ return a.poly.map(function(p){ return [p.x,p.y]; }); })
        : [];
    ...
    // C1 ölçüm: tam-kabuk blok sayısı + o bloklarda türetilen toplam pencere sayısı (perf raporu için)
    exteriorGroup.userData.fullBlocks=others.fulls.length;
    exteriorGroup.userData.blockWins=others.fulls.reduce(function(s,fb){ return s+(fb.protoWin||0); },0);
  ...
  function setExteriorMode(on){
    ...
    if(typeof extModeCallback==='function'){ try{ extModeCallback(exteriorMode); }catch(e){} }
  }
  ...
  function setExtBlockView(sel){
    ...
    // İŞ 3a: null da geçerli değer (varsayılana dön — 2B aktif blok tam kabuk). +null TUZAĞI (Number(null)===0
    //   → yanlışlıkla blok-0 seçermiş gibi davranırdı) → sel==null AYRI dallandı.
    const nv = (sel==='all')? 'all' : (sel==null? null : +sel);
  ...
  // İŞ B2: floorGroupsForTest'in DIŞ CEPHE ikizi — kullanım (konut/ticari/otopark/sığınak) filtresi
  //   YOK (kabuk her kat için var, iç mekân yalnız konuta özel kısıt burada uygulanmaz); TÜM katları
  //   (0..totalFloors()-1) floorLayoutSig eşitliğine göre ARDIŞIK gruplar. sig=null (ziyaret edilmemiş)
  //   hiçbir komşusuyla gruplanmaz (floorGroupsForTest ile AYNI kural). Dönen: [{idxs:[k,...], sig, rep}]
  //   — rep = grubun TEMSİLCİ kat indeksi (İLK eleman; buildExterior per-kat proto'yu BU kattan kurar).
  //   Saf/headless: THREE/sahne/map YOK — yalnız floorLayoutSig okur (buildExterior B3'te kullanılır).
  function floorFacadeGroupsForTest(){
    const out=[];
    let total=0;
    try{ total=(typeof totalFloors==='function')?totalFloors():0; }catch(e){ total=0; }
    for(let k=0;k<total;k++){
      const sig=(typeof floorLayoutSig==='function')? floorLayoutSig(k) : null;
      const prev=out[out.length-1];
      if(prev && sig!=null && prev.sig===sig && prev.idxs[prev.idxs.length-1]===k-1) prev.idxs.push(k);
      else out.push({ idxs:[k], sig:sig });
    }
    out.forEach(function(g){ g.rep=g.idxs[0]; });
    return out;
  }
  ...
  // İŞ 4b: MUTLAK (bağlam-DUYARSIZ) drone yükseklik aralığı — paket import (setExteriorCameras) için.
  //   clampDroneY (yukarı) CANLI binanın extBox.topY'sine göre dar bir aralık verir; bu, KULLANICI slider'ı
  //   için doğru (o an açık bina bağlamında makul üst sınır) ama import'ta YANLIŞ: paket FARKLI bir bina
  //   yüksekliğinden export edilmiş olabilir → aynı geçerli drone Y'si, şimdiki (daha kısa) binada sessizce
  //   aşağı çekilirdi. Geniş mutlak aralık [0.5m, 500m] içe aktarılan değeri OLDUĞU GİBİ korur, yalnız
  //   gerçekten saçma (negatif/aşırı) değerleri törpüler.
  const EXT_DRONE_Y_ABS_MIN=0.5, EXT_DRONE_Y_ABS_MAX=500;
  function clampDroneYAbs(y){ return Math.max(EXT_DRONE_Y_ABS_MIN, Math.min(EXT_DRONE_Y_ABS_MAX, +y)); }
  ...
  // A2: DIŞ ŞERİT OTOMATİK KARE — drone yoksa binayı/siteyi çerçeveleyen 1 varsayılan drone yerleştir
  //   (kuşbakışı/perspektif). extBox merkezinden dışarı, çatı ~0.7'sinde, merkeze bakar. Zaten drone varsa NO-OP.
  //   Dönüş: toplam drone sayısı. "Siteyi/Binayı Boya" CTA'sı bunu çağırır → hemen render edilebilir kare olur.
  ...
  function startExtGhost(){
    if(!exteriorMode) return;
    cancelExtGhost();
    extActive=-1; extPlaceActive=true; extPlaceAction='none';   // A4: yeni ekleme, seçili-drone aim/move armını bırak
  ...
  //   Hedef artık extCenter()'a KİLİTLİ DEĞİL: bir kez aim/yaw edilince o drone bağımsız hedefe sahip olur.
  //   extPlaceAction: 'none' (nötr) · 'move' (zemine tık = taşı) · 'aim' (zemine tık = o noktaya baktır).
  //   İç placeAction ikizi ama DIŞ moda özel (ayrık durum). Tekerlek her seçili drone'da yaw döndürür.
  function extYawDrone(deltaDeg){
  ...
  function extMoveDroneTo(hit){
    if(extActive<0||extActive>=extCams.length||!hit) return; const c=extCams[extActive];
    c.pos.x=hit.x; c.pos.z=hit.z;
    if(!c.aimed){ c.target=Object.assign({},extCenter()); }   // A4: aim edilmemişse taşınırken merkeze bakmaya devam
    renderExtGizmos(); renderExtDock();
    setHint&&setHint('Drone taşındı · tekrar tıkla = yine taşı · yön için "Yön"'); }
  function setExtPlaceAction(a){
    if((a==='aim'||a==='move') && extActive<0){ setHint&&setHint('Önce bir drone seç.'); return; }
    extPlaceAction=a||'none';
    if(controls){ controls.enabled=(extPlaceAction==='none'); }   // aim/move açıkken orbit kilitli (zemin-tık net)
    renderExtGizmos(); renderExtDock();
    setHint&&setHint(extPlaceAction==='aim'?'Zemine tıkla → seçili drone oraya bakar (tekerlek de döndürür).'
      :extPlaceAction==='move'?'Zemine tıkla → seçili drone oraya taşınır.'
      :'Drone: sürükle taşı · tekerlek/Yön ile bakışı ayarla · Yükseklik/Objektif dock\'ta.'); }
  ...
  // A3: SAHNE ENVANTERİ — state'ten OLGUSAL kısa İngilizce metin (blok sayısı + her blokun kat sayısı +
  //   cephe preset sinyali + imkanlar + avlu + düz teras çatı). YARATICI dış-render'da prompt'a ÇAPA olarak
  //   girer (AI kat/blok sayısını uydurmaz); SADAKAT'te bayrak KAPALI (img2img zaten kütleyi gösterir).
  //   opt.blocks (test) verilirse onu kullanır; yoksa siteOn()+blocks[] global'inden türetir. Headless güvenli.
  function extBlockFloorList(opt){
    // dönüş: [{floors:N}] blok başına (site kapalı → tek eleman = extFloorCount.above)
    if(opt && Array.isArray(opt.blocks) && opt.blocks.length){
      return opt.blocks.map(function(b){ return { floors:Math.max(1,(b&&b.floors)|0||1) }; });
    }
    const out=[];
    try{
      if(typeof siteOn==='function' && siteOn() && typeof blocks!=='undefined' && Array.isArray(blocks) && blocks.length){
        blocks.forEach(function(b){
          let fl=0; if(b&&b.ui){ fl=parseInt(b.ui.katSayisi)||0; }
          if(!fl) fl=extFloorCount().above;
          out.push({ floors:Math.max(1,fl|0) });
        });
      }
    }catch(e){}
    if(!out.length){ out.push({ floors:extFloorCount().above }); }   // tek bina (site kapalı / güvenli düşüş)
    return out;
  }
  function ordinalWord(n){ const w=['','one','two','three','four','five','six','seven','eight']; return w[n]||String(n); }
  function blockLetter(i){ return String.fromCharCode(65+i); }   // 0→A, 1→B ...
  function buildSceneInventory(opt){
    opt=opt||{};
    const bl=extBlockFloorList(opt);
    const n=bl.length;
    let s='';
    if(n>1){
      s+='The site has '+ordinalWord(n)+' residential blocks: '+
        bl.map(function(b,i){ return 'block '+blockLetter(i)+' '+b.floors+' floor'+(b.floors===1?'':'s'); }).join(', ')+'. ';
    } else {
    ...
    const am=amenityTypesPresent(opt);
    if(am.length){
      const phr=am.map(function(k){ return AMENITY_PHRASE[k]; });
      let joined = (phr.length===1)?phr[0]:(phr.slice(0,-1).join(', ')+' and '+phr[phr.length-1]);
      s+='Landscaped garden with '+joined+'. ';
    }
    // avlu (varsa)
    let hasCourt=false;
    try{ if(opt.courtyard!=null) hasCourt=!!opt.courtyard; else hasCourt=extCourtyards().length>0; }catch(e){}
  ...
  function buildExteriorPrompt(opt){
    ...
    if(opt.creative){
      // A2/A3: YARATICI şık — envanter çapası + serbest çevre/peyzaj. Kütle/kat sayısı envanterle bağlanır
      //   ama pencere-pencere LOCK yok (AI cephe/peyzaj yorumlar). no people/text korunur.
      return (
        'Photorealistic architectural exterior photograph, aerial drone perspective, of this residential '+
  ...
  //   İŞ 4a: aimed alanı taşır (drone hedefi bağımsız mı — extYawDrone/extAimDroneTo/setExtDroneTilt'te
  //   c.aimed=true olur) → paket roundtrip'te setExteriorCameras bunu geri okur (import sonrası "merkeze
  //   takip" davranışı sessizce kaybolmasın).
  function extCamViewObj(c,i){
    return { id:'ext'+(i+1), pos_m:{x:+c.pos.x.toFixed(3),y:+c.pos.y.toFixed(3),z:+c.pos.z.toFixed(3)},
      ...
      heading_deg:extHeadingOf(c), lens_mm:c.lens, fov_deg:lensToFov(c.lens), aimed:!!c.aimed,
  ...
  //   renderStyle: 'faithful' (LOCK — mevcut, varsayılan) · 'creative' (envanter çapası + serbest yorum, A2/A3).
  //   İŞ 1b: yalnız bağlam-eşleşen drone'lar (id ORİJİNAL extCams konumuna göre sabit — exportCameras ile aynı disiplin).
  ...
  function renderExtDock(){
    ...
    if(c){
      ...
      selRow='<div class="sep"></div>'+
        '<div class="col"><span class="lbl">Düzenle</span>'+
          '<div class="row">'+
  ...
  let extRenderConfirm=null;   // function(payload){ return Promise<bool|string> } — prototip atar (modal); string dönerse render-style
  let extRenderSubmit=null;    // function(payload){} — onaydan sonra render-server'a POST (prototip atar)
  //   arg renderStyle: 'faithful' (varsayılan) · 'creative'. Prototip modal'ı seçimi confirm callback dönüşünde
  //   ('faithful'|'creative' string) ya da doğrudan bu argümanla verebilir. payload buna göre kurulur.
  function triggerExteriorRender(renderStyle){
    ...
    function buildPayload(rs){
      const creative=(rs==='creative');
      return { profile:'exterior', style:extFacade, facade:extFacade, timeOfDay:timeOfDay,
        render_style:creative?'creative':'faithful', count:extCams.length, cameras:exportExteriorCameras(rs) };
    }
    const doSubmit=function(res){
      // res: true/false (eski sözleşme) YA DA 'faithful'/'creative' (yeni: seçilen stil)
      if(res===false || res==null) return false;
      const rs=(res==='creative')?'creative':(res==='faithful'?'faithful':(renderStyle||'faithful'));
      const payload=buildPayload(rs);
      if(typeof extRenderSubmit==='function'){ try{ extRenderSubmit(payload); }catch(e){} }
      return true;
    };
    if(typeof extRenderConfirm==='function'){ return Promise.resolve(extRenderConfirm(buildPayload(renderStyle||'faithful'))).then(doSubmit); }
    // standalone fallback: tarayıcı confirm (prototip modal atamadıysa)
    let okFlag=false;
    try{ okFlag=(typeof confirm==='function') ? confirm('Dış render başlatılsın mı?\n\n'+extCams.length+' drone kamerası, cephe: '+facadePreset(extFacade).name+'.\nBu işlem render kredisi harcar.') : true; }catch(e){ okFlag=true; }
    return Promise.resolve(doSubmit(okFlag ? (renderStyle||'faithful') : false));
  }
  ...
  function hideChromeForWalk(hide){
    ...
    ['#v3dDock','#v3dCamDock','#v3dFurnDock','#v3dMatDock','#v3dFurnBar','#v3dCamBar','#v3dPip','#v3dCompareThumb','#v3dLockBtn','#v3dViewLockWrap','#v3dZoomBar','#v3dStatus','#v3dOrb','#v3dPegman']
  ...
  function addWalkCamera(){
    ...
    camList.push(stampCamCtx(c)); activeCamIdx=camList.length-1;         // yeni kamera seçili → çıkışta şeritte+PiP'te hazır
    ...
    if(camPreviewMode && !camUIEnabled){
      if(!exteriorMode){
        camList.forEach(function(c,i){
          if(!camCtxMatch(c)) return;   // İŞ 1b: başka kat/blok bağlamının kamerası önizlemede görünmez
  ...
  function scenePick(ev){
    ...
    if(placeAction==='aim' && activeCamIdx>=0){             // seçili kamerayı yeni noktaya çevir (#4 açı düzenleme)
      ...
      if(!pendingPos){ pendingPos={x:p.x,z:p.z}; renderCamGizmos(); setHint('Şimdi kameranın BAKACAĞI noktaya tıkla'); }
        ...
        camList.push(stampCamCtx(c)); activeCamIdx=camList.length-1; pendingPos=null; placeAction='none';   // KAMERA-S2: eklenince NÖTR (stray boş-tık aim etmesin)
        renderCamGizmos(); syncCamBtns(); logRoom(c);
  ...
  function clickCamGhost(ev){
    ...
    if(camGhost.phase==='pos'){
      ...
      camList.push(stampCamCtx(c)); activeCamIdx=camList.length-1; pendingPos=null; placeAction='aim';
  ...
  function attachPicker(){
    ...
    el.addEventListener('pointerup',function(e){
      ...
      if(exteriorMode){   // S2: dış mod — drone sürükle bitir / hayalet yerinde tık = drone düşür
        ...
        // A4: Yön/Taşı armlıysa boş-zemin tık = seçili drone'u baktır/taşı (iç scenePick aim/move ikizi)
        if(!moved && e.button===0 && extActive>=0 && (extPlaceAction==='aim'||extPlaceAction==='move')){
    ...
    el.addEventListener('wheel',function(e){ if(furnMode && activeFurnIdx>=0){ const f=furnList[activeFurnIdx], st=e.shiftKey?1:5;
      ...
      // A4: dış modda seçili drone'da tekerlek = yaw döndür (iç kamera yawCam ikizi; 5°/Shift 1°). Orbit zoom atlanır.
      if(exteriorMode && extActive>=0 && !extGhost && !extDrag){ const st=e.shiftKey?1:5; extYawDrone(e.deltaY<0?st:-st); return; }
  ...
  //   İŞ 1b: yalnız CANLI bağlama (camCtxMatch) eşleşen kameralar işlenir — başka kat/blokta konan kamera
  //   sızmaz. id numaralandırması ORİJİNAL camList konumuna göre SABİT kalır (i = tam listedeki index, filtre
  //   SONRASI yeniden numaralanmaz) — mesken/02_PROTOTIP pkgCollectCameras id-eşleştirmesi + İş 2 blok-başına
  //   yakalamada aynı kameranın id'si bloklar arası ÇAKIŞMAZ/KAYMAZ (bkz. o dosyadaki pkgMergeCam yorumu).
  function exportCameras(map){
    ...
    const out=[];
    camList.forEach(function(c,i){
      if(!camCtxMatch(c)) return;
      const o=camViewObj(c,i,map);
      o.render_method=camRenderMethod;
      o.time_of_day=camTime(c);                                                                  // B1-4: gün saati (override yoksa global)
      if(wantPrompt) o.prompt=(c.prompt!=null&&c.prompt!=='')?c.prompt:autoCamPrompt(c,i,map);   // opsiyon 1: metin
      if(wantSnap)   o.snapshot=snapCameraDataURL(c);                                             // opsiyon 2: kendi-açı PNG
      out.push(o);
    });
    return out;
  }
  ...
  // İŞ 1b: yalnız bağlam-eşleşen kameraların thumbnail'i (id/idx ORİJİNAL camList konumuna göre sabit — exportCameras ile aynı disiplin).
  ...
  window.View3D = { open:open, openCompare:openCompare, openPlace:openPlace, close:close, snapDataURL:snapDataURL, getView:getView, restoreView:restoreView,
    ...
    // İŞ 3a: extBlockView (F4 dış-mod GÖRÜNTÜ-only blok seçimi) PUBLIC API — mevcut iç setExtBlockView'i
    //   sarar (exteriorMode kapalıyken set no-op, mevcut guard KORUNUR; null da geçerli değer — varsayılana
    //   dön). Amaç: buildScene'in S1 resetine (~1245: her rebuildFromEngine'de extBlockView=null) karşı
    //   prototip tarafında SAKLA+GERİ-UYGULA (bkz. mesken/02_PROTOTIP captureRenderFrames — İş 2'nin blok
    ...
    //   opt:{label,title,titleOn} — buton tooltip'i. fn(view|null) tıklamada çağrılır (toggle).
    setViewLockCallback:function(fn,opt){ setViewLockCallback(fn,opt); },
    // dış senkron: buton görsel durumunu (kilitli/kilitsiz) çağıranın state'ine göre düzelt (basit tut — iç toggle'ı da günceller).
    setViewLockState:function(on){ setViewLockState(on); },
    ...
    // A2: dış şerit otomatik kare — drone yoksa binayı/siteyi çerçeveleyen varsayılan drone yerleştir (kuşbakışı).
    deriveShowcaseDrones:deriveShowcaseDrones,
    ...
    //   Minimal salt-veri sahne-güdüğü: {__cx:0,__cz:0,__map} — worldToPx/snapCameraDataURL'in null-güvenli
    //   dallarını (renderer yok → snapshot:null) tetikler, gerçek THREE render'a GİRMEZ.
    //   map verilmezse (null/undefined) scene GERÇEK null'a döner (camListForTest/extCamsForTest gibi
    //   renderCamGizmos/renderExtGizmos ÇAĞIRAN yollar THREE olmadan güvenle no-op kalsın — güdük SADECE
    //   exportCameras/snapCameraThumbs çağrısından HEMEN ÖNCE kurulmalı).
    camSceneStubForTest:function(map){ scene = map ? {__cx:0,__cz:0,__map:map} : null; },
    ...
    extAimDroneForTest:function(hit){ extAimDroneTo(hit); return (extActive>=0?Object.assign({},extCams[extActive].target):null); },
    extMoveDroneForTest:function(hit){ extMoveDroneTo(hit); return (extActive>=0?Object.assign({},extCams[extActive].pos):null); },
    extSelectForTest:function(i){ selectExtCam(i); return extActive; },
    extSetPlaceActionForTest:function(a){ setExtPlaceAction(a); return extPlaceAction; },
