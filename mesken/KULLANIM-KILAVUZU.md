...
*"Mevzuat Kontrolü" penceresi: Tamam / Sorunlar / Bilgi katlanır bölümleri (görseldeki sorun satırı örnektir; güncel demo projesi mevzuat-temizdir, Sorunlar 0 gösterir); bölgeli satıra tıklamak tuvalde o alana yakınlaştırır.*
...
2. **"Bakış yönünü değiştir"** — tur senin için bir kamerayı seçer. Adım **iki aşamalıdır**: **önce** alttaki dock'ta işaretli **"Yön"** düğmesine bas (yön/aim moduna girersin), **sonra** sahnede kameranın bakacağı noktaya tıkla (koni ucunu sürükleyerek ya da tekerlekle de çevirebilirsin). Sahne bu adımda **kararmaz** — hedef düğme deliğine ek olarak sahne tuvali de aydınlık ve tıklanabilir kalır. Seçili kamera **parlak turuncu marker + nabız halkasıyla** belirginleşir (hangi kamerayı düzenlediğin görülür). Sahnede **başka bir kamerayı seçmen adımı ilerletmez** — yalnız **seçili** kameranın bakış yönü gerçekten değişince adım geçer.
...
- Yalnız hiç seçim yokken (ya da oda türü tanınmadığında) **genel liste** görünür. Bu genel Döşe listesindeki satırlar da bağlamsal listelerle **birebir aynı davranır**: her satırda aynı **aktif "Değiştir"** düğmesi vardır, tıklayınca "Alternatifler" listesini açar/kapar (yalnız bilgilendiren bildirim; seçimi değiştirmez). Ürünler, markalar ve fiyatlar **tamamen örnektir** (placeholder) — gerçek bir katalog/ağ bağlantısı yoktur.
