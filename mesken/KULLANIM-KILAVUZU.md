...
- **"Proje İndir"** — bir yerleşimin varsa projeni `proje-YYYYAAGG.mskpkg` olarak indirir **ve** üretilmiş render'ları ayrıca birer JPEG olarak kaydeder (iç kareler kamera adıyla, dış kareler `dis-N`, plan boyama `plan`/`plan-blok`). Yalnız parsel/sınır çizip yerleşim üretmediysen paket anlamsız olacağından motorun SVG'sini indirir (yine "Proje Aç" ile geri açılır). **Parsel ya da bina sınırını çizer çizmez aktifleşir** — yerleşim (Adım 2) beklemez; öncesinde düğme pasiftir ("Önce bir parsel ya da bina sınırı çizin"). Bitişik **▾** menüsündeki **"2B DXF indir"** yalnız yerleşim varken etkindir.
...
Üretilen render'lar orta alanda **tek düzenli, blok-bölümlü bir kart akışında** toplanır — kartlar Render adımıyla (Adım 4) aynı dildedir (aynı genişlik/oran/köşe/gölge). Adımın CTA'sı **"Projeyi İndir"** — titlebar'daki "Proje İndir" ile aynı akışı çağırır (.mskpkg + render JPEG'leri).
